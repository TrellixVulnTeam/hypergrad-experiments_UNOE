...
def download_dataset(path, source='https://www.cs.toronto.edu/~kriz/'
    ...
    for fn in files:
        if not os.path.exists(os.path.join(path, 'cifar-10-batches-py', fn)):
            print(os.path.join(path, 'cifar-10-batches-py', fn))
...
def load_dataset(params, path='datasets'):
    ...
    data = [np.load(os.path.join(path, 'cifar-10-batches-py',
                                 'data_batch_%d' % (i + 1)), encoding='latin1') for i in range(5)]
    X_train = np.vstack([d['data'] for d in data])
    y_train = np.hstack([np.asarray(d['labels'], np.int8) for d in data])

    # test data
    data = np.load(os.path.join(path, 'cifar-10-batches-py', 'test_batch'), encoding='latin1')
    X_test = data['data']
    ...
    # normalize
    try:
        mean_std = np.load(os.path.join(path, 'cifar-10-mean_std.npz'), encoding='latin1')
        mean = mean_std['mean']
        std = mean_std['std']
    except IOError:
