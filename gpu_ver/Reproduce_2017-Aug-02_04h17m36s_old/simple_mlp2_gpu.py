...
from updates import update, updates_hyper, update2
...
"""
Read in and preprocess the CIFAR10 data
"""
...
#TODO: Move data from test to valid set; is this compatible with preprocessing and fair evaluations?
X_mix = np.concatenate((X_valid, X_test), axis=0) #valid first
Y_mix = np.concatenate((Y_valid, Y_test), axis=0)
print(X_valid.shape, X_mix.shape)
X_valid, Y_valid = X_mix[:len(X_mix) // 2], Y_mix[:len(X_mix) // 2] #valid first again
X_test, Y_test = X_mix[len(X_mix) // 2:], Y_mix[len(X_mix) // 2:]
#TODO: X_valid, Y_valid is never actually used
"""
...
#TODO: this uses Nesterov momentum and a common learning rate for all elementary parameters
...
func_hyper_valid = theano.function(
    ...
    updates=update_valid, #TODO: this updates output_valid_list to dloss_dweight = T.grad(model.loss, model.params_weight)
    on_unused_input='ignore',
    allow_input_downcast=True)
    #TODO: output_valid_list in the outputs returns an array corresponding to the previous value
...
# Phase 3
#TODO: output_valid_list here should be dv_t
...
#TODO: below will be lists of lists. 
...
#TODO: use random batches for errors/losses, not the same batches predictably
...
#TODO: Luketina+learning rates; don't restart batches from first
...
#TODO: make a directory with a timestamp name, and save results to it
import os
print("---------------------------------------------------------------------------------------------------")
...
#TODO: copy files to directory in case of changes
import shutil
...
#def run_exp(args, update_lambda, update_llrs, fix_weight, epochs):
    ...
    if fix_weight:
        ...
        for fix, origin, velocity in zip(fix_weight, model.params_theta, velocities):
            ...
            #TODO: don't reset velocities for online updates
    ...
    #TODO: this could be put outside of run_exp()
        ...
        for origin in model.params_theta:
            fix_weight.append(origin.get_value()) #copies
    #TODO: Or we can keep the final weights from before, as in Luketina (and hypergradient descent for learning rates)
    # Or only keep the ScaleLayer parameters? 

    #TODO: Log learning rates
    ...
    n_batch_ele = n_ele // args.batchSizeEle #TODO: integer division, so some data may be lost
    #TODO: 
    #n_batch_ele = 100
    ...
    #T = 20
    for i in range(0, T): # Michael: SGD steps
        ...
        # Michael: This is where the elementary parameters are updated
        train_loss, unreg_train_loss, train_pred = func_elementary(batch_x, batch_y, lr_ele_true) #TODO: no lr_ele_true for lr optimization

        if i > 3 and i < min(20,T):
        ...
        if i%20==0:
            ...
            for e1, e2 in zip(train_pred, Y_elementary[sample_idx_ele]): #
                if e1 != e2:
                    wrong += 1
            train_error = 100. * wrong / len(train_pred) #TODO:
        ...
        if curr_batch == n_batch_ele - 1: #last batch
            #TODO: use deterministic option for BN layers
    ...
    for j, i in enumerate(random_sample(range(n_batch_hyper), k=n_batch_hyper // 5)): #~15 batches covering about 5000/5 = 1000
    ...
    err_valid /= (n_batch_hyper // 5) #TODO:
    cost_valid /= (n_batch_hyper // 5) #TODO:
    ...
    for i, grad in enumerate(grad_l_theta):
        ...
        #TODO:
        grad_l_theta[i] = grad / (np.array((n_batch_hyper // 5) * 1., dtype=theano.config.floatX))
        print("grad_l_theta_norm", np.linalg.norm(grad_l_theta[i]))
    ...
    # Maybe don't use the full validation set above? But make sure to use all of it over different meta-iters.
    ...
    #TODO: calculate number of reverse iterations
    ...
    #TODO: beta = 10.*beta in CPU version...
...
def update_lambda_every_meta(ori, up, hyper_lr, updatemode=None): #returns the new hyperparameter values
    tmp = []
    for x, y in zip(ori, up):
        if updatemode == 'unit':
            new_y = np.mean(y, axis=1, keepdims=True) #TODO: average over axis 1, which corresponds to the unit? 
            #For conv layer, axis=1 is the input channels, so axis=0 for the filters would make more sense
...
def update_lambda_every_meta_mom(ori, up, lam_vels, hyper_lr, hyper_mom, updatemode=None): #returns the new hyperparameter values
    tmp = []
...
def update_lambda_lr_every_meta(ori, up, llr, up_llr, hyper_lr, updatemode=None):
    tmp = []
    for x, y in zip(ori, up):
        if updatemode == 'unit':
            new_y = np.mean(y, axis=1, keepdims=True)
    ...
    for x, y in zip(llr, up_llr):
        if updatemode == 'unit':
            new_y = np.mean(y, axis=1, keepdims=True)
            tmp2.append(x.get_value() - np.sign(new_y) * np.array(float(hyper_lr) * 1., dtype=theano.config.floatX))
            #print("metaupdate", x.get_value()[0][1], tmp[-1][0][1])
        else: #per parameter
            tmp2.append(x.get_value() - np.sign(y) * np.array(float(hyper_lr) * 1., dtype=theano.config.floatX))
    return tmp, tmp2


def update_lambda_lr_every_meta_mom(ori, up, lam_vels, llr, up_llr, llr_vels, hyper_lr, hyper_mom, llr_mom, updatemode=None):
    tmp = []
    hyper_mom = np.array(float(hyper_mom) * 1., dtype=theano.config.floatX)
    for x, y, v in zip(ori, up, lam_vels):
        if updatemode == 'unit':
    ...
    for x, y, v in zip(llr, up_llr, llr_vels):
        if updatemode == 'unit':
            ...
            tmp2.append(x.get_value(borrow=True) + v.get_value(borrow=True) * np.array(float(hyper_lr) * 1., dtype=theano.config.floatX))
        else: #per parameter
...
for i in range(args.metaEpoch):
    ...
    #TODO: save model parameters, hyperparameters, hyperparameter velocities
    ...
    print("Test error: "+str(test_errors[-1]))
    print("---------------------------------------------------------------------------------------------------")
