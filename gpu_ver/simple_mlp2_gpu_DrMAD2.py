...
import os
print("---------------------------------------------------------------------------------------------------")
print('DrMAD2_{:%Y-%b-%d_%Hh%Mm%Ss}'.format(datetime.datetime.now()))
print("---------------------------------------------------------------------------------------------------")
outdir = 'DrMAD2_{:%Y-%b-%d_%Hh%Mm%Ss}'.format(datetime.datetime.now())
os.mkdir(outdir)
...
#exact reverse path functions
updates_reverse_params = []
for velocity, param in zip(velocities, model.params_theta):
    updates_reverse_params.append((param, param - lr_ele * velocity))
func_reverse_params = theano.function(inputs=[lr_ele], updates=updates_reverse_params)
...
#def run_exp(args, update_lambda, update_llrs, fix_weight, epochs):
    ...
    for i in range(0, T): #SGD steps
        ...
        if curr_epoch >= 150: #30
            lr_ele_true = args.lrEle/10.
        elif curr_epoch >= 225: #60
    ...
    f.close()
    theta_final = None #TODO: empty to save space on the GPU
    ...
    checkpoint = [param.get_value(borrow=False) for param in model.params_theta]
    ...
    beta = beta/sum([np.linalg.norm(param.get_value()-param0) for param, param0 in zip(model.params_theta, theta_initial)])
    ...
    #n_backward = min(n_backward, 2*args.nReversedIters)
    #print("n_backward", n_backward)
    
    #n_backward = 2*args.nReversedIters
    ...
    def replace_pseudo_params(ratio):
        for i, param in enumerate(model.params_theta):
            pseudo_params[i] = (1 - ratio) * theta_initial[i] + ratio * checkpoint[i]
    ...
    rand_batches = random_sample(range(n_batch_ele), n_backward)
    ...
    for iteration in range(n_backward)[::-1]:
        ...
        #sample_idx_ele = iter_index_cache[iteration] # TODO: should we use the same batches? The number of iterations doesn't correspond anymore
        sample_idx_ele = ele_perm[(rand_batches[iteration] * args.batchSizeEle):((rand_batches[iteration] + 1) * args.batchSizeEle)]
        # sample_idx_ele = ele_perm[(curr_batch * args.batchSizeEle):((curr_batch + 1) * args.batchSizeEle)]
        batch_x, batch_y = X_elementary[sample_idx_ele], Y_elementary[sample_idx_ele]
        tmp_y = np.zeros((args.batchSizeEle, 10))
        for idx, element in enumerate(batch_y):
...
def update_lambda_every_meta(ori, up, hyper_lr, updatemode='output_unit'): #returns the new hyperparameter values
    ...
    for x, y in zip(ori, up):
        ...
        elif updatemode == 'layer':
            new_y = np.mean(y, keepdims=True)
        else: #per parameter
            new_y = y
...
for i in range(args.metaEpoch):
    ...
    #TODO: per-layer hyperparameters    
    temp_lambda = update_lambda_every_meta_mom(model.params_lambda, temp_lambda, lambda_velocities, args.lrHyper, args.momHyper, updatemode='layer')
