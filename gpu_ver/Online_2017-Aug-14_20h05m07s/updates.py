...
#TODO: the velocity includes the learning rate...
...
def update(params_theta, params_lambda, params_weight, velocities, loss, penalty, lossWithPenalty,
    ...
    if alg == 'SGDNesterov':
        ...
        update_ele = custom_mom(lossWithPenalty, params_theta, velocities, lr_ele, momentum=mom)
    else:
        raise AssertionError

    #The rest is used for grad_l_theta = dw_T = gradient of validation loss wrt final weights
    dloss_dweight = get_or_compute_grads(loss, params_weight)
...
def update2(params_theta, params_lambda, params_weight, velocities, loss, penalty, lossWithPenalty,
    ...
    #The rest is used for grad_l_theta = dw_T = gradient of validation loss wrt final weights
    dloss_dtheta = get_or_compute_grads(loss, params_theta)
...
def updates_hyper(params_lambda, params_weight, lossWithPenalty, share_var_dloss_dweight):
    ...
    dLoss_dweight = get_or_compute_grads(lossWithPenalty, params_weight) #T.grad(lossWithPenalty, params_weight) # the TensorVariable, no value yet
...
def updates_hyper2(params_lambda, params_theta, params_weight, lossWithPenalty, share_var_dloss_dtheta):
    ...
    HVP_theta = []
    HVP_lambda = []
    dLoss_dtheta = get_or_compute_grads(lossWithPenalty, params_theta)
...
def updates_noise_hyper(params_noise, params_theta, lossWithPenalty, share_var_dloss_dtheta):
    update_hyper = []
    HVP_noise = []
    dLoss_dtheta = get_or_compute_grads(lossWithPenalty, params_theta)
    HVP_noise_temp = Lop2(dLoss_dtheta, params_noise, share_var_dloss_dtheta)
    for param, grad in zip(params_noise, HVP_noise_temp):
        save_grad = theano.shared(np.asarray(param.get_value() * 0., dtype=theano.config.floatX),
                                  name='HVP_lambda_'+param.name, broadcastable=param.broadcastable)
        update_hyper += [(save_grad, grad)]
        HVP_noise += [save_grad]
    return update_hyper, HVP_noise #, share_var_dloss_dtheta