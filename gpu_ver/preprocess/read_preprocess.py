# Michael: added parentheses to print statements

import cPickle
#import _pickle as cPickle #Michael: for Python 3
import gzip
import os
...
def read(params):
    ...
    if params.dataset == 'mnist':
       ...
       for item in batchNames: 
           fo = open(folderName + item, 'rb'); dict = cPickle.load(fo, encoding='latin1'); fo.close()
           t1Data = np.append(t1Data, np.float32(dict['data']), axis = 0)
           t1Label = np.append(t1Label, np.int32(dict['labels']))
           
       fo = open(folderName + 'data_batch_5', 'rb'); dict = cPickle.load(fo, encoding='latin1'); fo.close()
       vData = np.float32(dict['data']); vLabel = np.int32(dict['labels'])  
       fo = open(folderName + 'test_batch', 'rb'); dict = cPickle.load(fo, encoding='latin1'); fo.close()
