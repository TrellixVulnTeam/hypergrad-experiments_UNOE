...
from updates import update, updates_hyper, updates_hyper2, update2
...
X_hyper = X_elementary[0:10000]
Y_hyper = Y_elementary[0:10000]
X_elementary = X_elementary[10000:]
Y_elementary = Y_elementary[10000:]
...
update_hyper, HVP_theta, HVP_lambda, phase_3_input = updates_hyper2(model.params_lambda, model.params_theta, 
                                                                               model.params_weight, model.lossWithPenalty, 
                                                                               output_valid_list)
...
def run_exp(args, update_lambda, update_llrs, first_iter):
    ...
    n_batch_samples_hyper = 10 # T #TODO: 
    ...
    for iteration in range(0, T_rev)[::-1]:
    ...
    return up_lambda, up_log_learning_rates


# clip llrs between -104 and 0 (np.float32(np.e**-104.) == 0.)

def update_lambda_lr_every_meta(ori, up, llr, up_llr, hyper_lr, lr_Llr, updatemode='output_unit'):
    tmp = []
    ...
    for x, y in zip(llr, up_llr):
        if updatemode == 'unit' or (updatemode == 'output_unit' and x.name == 'output.W_llr'):
            new_y = np.mean(y, axis=1, keepdims=True)
            tmp2.append(np.clip(x.get_value() - np.sign(new_y) * np.array(float(lr_Llr) * 1., dtype=theano.config.floatX), a_min = -16., a_max=0.))
            #print("metaupdate", x.get_value()[0][1], tmp[-1][0][1])
            #print('shared llr')
        else: #per parameter
            tmp2.append(np.clip(x.get_value() - np.sign(y) * np.array(float(lr_Llr) * 1., dtype=theano.config.floatX), a_min = -16., a_max=0.))
    return tmp, tmp2
...
for metameta in range(args.metaEpoch):
    ...
    while ele_iter < epochs*n_batch_ele:
        temp_lambda, temp_llrs = run_exp(args, temp_lambda, temp_llrs, ele_iter)
        ele_iter = ele_iter+args.onlineItersPerUpdate
        if args.momLlr == 0.:
            temp_lambda, temp_llrs = update_lambda_lr_every_meta(model.params_lambda, temp_lambda, log_learning_rates, temp_llrs, args.lrHyper, args.lrLlr)
        else:
    ...
    if metameta < args.metaEpoch - 1: #for all but the last meta-meta-iteration
        ...
        #median rather than average for robustness, and simply because it minimizes the total distance, rather than squared distance, so it should be fastest to return to values before taking median?
            lamb.set_value(np.full_like(lamb.get_value(), np.median(lamb.get_value())))
        ...
        for j, reg in enumerate(model.params_lambda):
            per_layer_log_reg[j].append(np.mean(reg.get_value()))
