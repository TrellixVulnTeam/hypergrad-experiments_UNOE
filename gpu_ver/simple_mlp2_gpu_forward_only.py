...
#TODO: also set seed here to reproduce dataset exactly?
...
lasagne.random.get_rng().seed(args.seed) #reset the seed; weight initialization used randomness, too
...
def run_exp(args, fix_weight, theta_final, epochs):
    #lasagne.random.get_rng().seed(args.seed)
    #model = DenseNet(x=x, y=y, args=args)
...
for p1, p2 in zip(model.params_theta, tmp_weights):
    p1.set_value(p2)
#reset seed and reinitialize dropout layers
lasagne.random.get_rng().seed(args.seed)
for layer in model.layers: 
    if 'Dropout' in str(layer):
        layer.reinit()
