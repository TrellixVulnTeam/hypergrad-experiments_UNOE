...
def make_nn_funs(layer_sizes):
    ...
    def predictions(W_vect, X):
        ...
        for i in range(N_iter): #layer-wise
            cur_W = W[('weights', i)]
            cur_B = W[('biases', i)]
            cur_units = np.dot(cur_units, cur_W) + cur_B
            if i == (N_iter - 1): #Michael: last layer, for prediction
                cur_units = cur_units - logsumexp(cur_units, axis=1)
            else:
                cur_units = np.tanh(cur_units)
                # Michael: ReLU leads to weights increasing without bound
                # Is the ReLU derivative implemented?
                # cur_units = np.maximum(0,cur_units)
        return cur_units

    def loss(W_vect, X, T, L2_reg=0.0):
        # TODO: consider treating L2_reg as a matrix
        # Michael: L2_reg can be a vector and this will work fine?
        log_prior = -np.dot(W_vect * L2_reg, W_vect) #TODO: biases are regularized? 
