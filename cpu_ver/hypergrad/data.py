...
def datapath(fname):
    datadir = os.path.abspath('../../data/mnist')
...
def load_data(normalize=False):
    with open(os.path.join(os.path.abspath('../../data/mnist'), 'mnist_data.pkl')) as f:
...
def load_data_subset(*args):
    ...
    all_images = np.concatenate((train_images, test_images), axis=0) #Michael: put them all together
    all_labels = np.concatenate((train_labels, test_labels), axis=0)
    #print(len(all_labels))
    datapairs = []
    start = 0
    for N in args: #TODO: this is where test, validation and training split happens. It looks correct.
        end = start + N
        #print(start)
        #print(end)
