...
def sgd_meta_only(L_grad, meta, x0, alpha, beta, N_iters,
    ...
    def hypergrad(outgrad):
        ...
        x_current = np.copy(X.val)
        v_current = np.copy(V.val)
        d_v2 = np.copy(d_v)
        d_meta2 = np.copy(d_meta)
        d_x2 = np.copy(d_x)
        
        for i in range(N_iters)[::-1]:
            X.sub(alpha * V.val)  # Reverse position update
            x_current = x_current - alpha*v_current
            x_distances.append(np.linalg.norm(X.val-x_current))

            # add distance
            #dist_from_x_final.append(np.linalg.norm(X.val-x_final))            
            
            g = L_grad(X.val, meta, i)  # Evaluate gradient
            g2 = L_grad(x_current, meta, i)
            
            V.add((1.0 - beta) * g).div(beta)  # Reverse momentum update
            v_current = (v_current+(1.0-beta)*g2)/beta
            v_distances.append(np.linalg.norm(V.val-v_current))
            v_angles.append(np.dot(V.val, v_current)/(np.linalg.norm(V.val)*np.linalg.norm(v_current)))
            
            #d_v += d_x * alpha
            d_v = beta*d_v + d_x * alpha #correction?
            d_v2 = beta*d_v2 + d_x2 * alpha
            
            d_x -= (1.0 - beta) * L_hvp_x(X.val, meta, d_v, i)
            d_x2 -= (1.0 - beta) * L_hvp_x(x_current, meta, d_v2, i)
            d_x_distances.append(np.linalg.norm(d_x-d_x2))
            d_x_angles.append(np.dot(d_x, d_x2)/(np.linalg.norm(d_x)*np.linalg.norm(d_x2)))
            
            d_meta -= (1.0 - beta) * L_hvp_meta(X.val, meta, d_v, i)
            d_meta2 -= (1.0 - beta) * L_hvp_meta(x_current, meta, d_v2, i)
            hypergrad_distances.append(np.linalg.norm(d_meta-d_meta2))
            hypergrad_angles.append(np.dot(d_meta, d_meta2)/(np.linalg.norm(d_meta)*np.linalg.norm(d_meta2)))
            ...
            if i%40 == 0:
                #print(len(v_angles))
                #print(len(hypergrad_distances))
                #print(len(range(N_iters-1, N_iters-len(hypergrad_distances)-1,-1)))
                plt.plot(range(N_iters-1, N_iters-len(hypergrad_distances)-1,-1), v_angles, label="v")
                plt.plot(range(N_iters-1, N_iters-len(hypergrad_distances)-1,-1), hypergrad_angles, label="d_meta")
                plt.plot(range(N_iters-1, N_iters-len(hypergrad_distances)-1,-1), d_x_angles, label="d_x")
                plt.title("Cosine of angle between exact and without exact arithmetic per ele iteration")
                plt.xlabel("reverse elementary iteration")
                plt.ylabel("cosine of angle")
                plt.legend()
                plt.savefig("exact_angles"+str(N_iters)+"_"+str(meta_iteration)+"_corrected.png")
                plt.clf()
                
                plt.plot(range(N_iters-1, N_iters-len(hypergrad_distances)-1,-1), v_distances)
                plt.title("Distance between exact v and v without exact arithmetic per ele iteration")
                plt.xlabel("reverse elementary iteration")
                plt.ylabel("distance")
                plt.legend()
                plt.savefig("exact_v_distances"+str(N_iters)+"_"+str(meta_iteration)+"_corrected.png")
                plt.clf()
                
                plt.plot(range(N_iters-1, N_iters-len(hypergrad_distances)-1,-1), x_distances)
                plt.title("Distance between exact x and x without exact arithmetic per ele iteration")
                plt.xlabel("reverse elementary iteration")
                plt.ylabel("distance")
                plt.legend()
                plt.savefig("exact_x_distances"+str(N_iters)+"_"+str(meta_iteration)+"_corrected.png")
                plt.clf()
                
                plt.plot(range(N_iters-1, N_iters-len(hypergrad_distances)-1,-1), hypergrad_distances)
                plt.title("Distance between exact d_meta and d_meta without exact arithmetic per ele iteration")
                plt.xlabel("reverse elementary iteration")
                plt.ylabel("distance")
                plt.legend()
                plt.savefig("exact_d_meta_distances"+str(N_iters)+"_"+str(meta_iteration)+"_corrected.png")
                plt.clf()
                
                plt.plot(range(N_iters-1, N_iters-len(hypergrad_distances)-1,-1), d_x_distances)
                plt.title("Distance between exact d_x and d_x without exact arithmetic per ele iteration")
                plt.xlabel("reverse elementary iteration")
                plt.ylabel("distance")
                plt.legend()
                plt.savefig("exact_d_x_distances"+str(N_iters)+"_"+str(meta_iteration)+"_corrected.png")
                plt.clf()
        assert np.all(ExactRep(x0).val == X.val)
        ...
        #plt.savefig('exact_norms'+str(N_iters)+'_'+str(meta_iteration)+'_corrected.png')
        #plt.show()
        return d_meta
...
def sgd_meta_only_mad2(L_grad, meta, x0, alpha, gamma, N_iters,
                callback=None, forward_pass_only=True):
    x_init = x0
    x_current = x0
    v_current = np.zeros(x0.size)
    ...
    def hypergrad(outgrad,x_current=x_current, v_current=v_current):
        d_x = outgrad
        d_v, d_meta = np.zeros(d_x.shape), np.zeros(meta.shape)
        grad_proj = lambda x, meta, d, i: np.dot(L_grad(x, meta, i), d)
        L_hvp_x = grad(grad_proj, 0)  # Returns a size(x) output.
        ...
        bits_lost_per_mult = -np.log2(gamma)
        #but also bits lost by calculating gradient at wrong parameters
        #so assume at most three times as many bits are lost per iteration
        bits_lost_per_iter = 3.*bits_lost_per_mult
        #we have 23 precision bits; let's try to use at most 20 of them
        exact_iters = int(20./bits_lost_per_iter)
        print("exact_iters", exact_iters)
        for i in range(max(N_iters-exact_iters,0), N_iters)[::-1]:
            x_current -= alpha*v_current # Reverse position update
            g = L_grad(x_current, meta, i)  # Evaluate gradient
            v_current = (v_current+(1.0-gamma)*g)/gamma # Reverse momentum update
            #d_v += d_x * alpha
            d_v = gamma*d_v + d_x * alpha #correction?
            d_x -= (1.0 - gamma) * L_hvp_x(x_current, meta, d_v, i)
            d_meta -= (1.0 - gamma) * L_hvp_meta(x_current, meta, d_v, i)
        
        x_checkpoint = x_current
        ...
        for i in range(0,len(weights)):
            beta = beta + weights[i]*dist_from_x0[i]
        beta = 10.*beta/np.linalg.norm(x_checkpoint-x0)
        N_safe_sampling = int(np.ceil(1./beta)) #max(N,int(np.ceil(1./beta)))
        #print("weights", weights)
        #print("dist_from_x0", dist_from_x0)
        print("N_safe_sampling2", N_safe_sampling)
        N_safe_sampling = min(N_safe_sampling, max(N_iters-exact_iters,0))
        print("N_safe_sampling2_2", N_safe_sampling)
        
        beta = np.linspace(0.,1.,N_safe_sampling)[0:N_safe_sampling-1] #checkpoint was already used above
        for i in range(N_safe_sampling-1)[::-1]:
            x_current = (1-beta[i])*x_init + beta[i]*x_checkpoint
            #d_v += d_x * alpha
            d_v = gamma*d_v + d_x * alpha #correction?
            d_x -= (1.0 - gamma) * L_hvp_x(x_current, meta, d_v, i)
            d_meta -= (1.0 - gamma) * L_hvp_meta(x_current, meta, d_v, i)
...
sgd_meta_only_mad2 = Differentiable(sgd_meta_only_mad2,
                               partial(sgd_meta_only_mad2, forward_pass_only=False))
