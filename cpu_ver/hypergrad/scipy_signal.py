...
def convolve(A, B, axes=None, dot_axes=[(),()], mode='full', forward_pass_only=True):
    ...
    def grad_convolve(A=A, B=B, axes=axes, dot_axes=dot_axes, mode=mode):
        ...
        #Michael: parameters in second argument
        X, Y = B, A
        _X_, _Y_ = 'B', 'A'
        ignore_Y = 'ignore_A'
...
def pad_to_full(A, B, axes):
    ...
    return np.pad(A, A_pad, mode='constant')
